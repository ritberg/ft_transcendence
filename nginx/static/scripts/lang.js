...
//fetches the selected language and translates the website
...
//fetches current language in the user settings
...
export async function fetchLanguage() {
    ...
    return await fetch(fetchLanguageUrl, {
        ...
        .then((data) => {
            if (data !== null) {
                console.log("language on db:", data);
...
//changes the default account language for a user
export async function changeLanguage(language) {
    ...
    //ensures updateStatus has time to send it's message
    await sleep(100);
