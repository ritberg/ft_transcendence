document.addEventListener("DOMContentLoaded", function () {
  ...
  let signupUrl = "http://localhost:8003/auth/register/";
  let updateUrl = "http://localhost:8003/auth/update/";
  let logoutURL = "http://localhost:8003/auth/logout/";
  let friendRequestUrl = "http://localhost:8003/auth/send-friend-request/";
  ...
  let delFriendUrl = "http://localhost:8003/auth/delete-friend/";
...
/////////////     (Un)Blocking users end (Rita)  //////////////
  ...
  const getUserId = async (username) => {
    const response = await fetch(
      `http://localhost:8003/auth/get-user-id/?username=${username}`
    );
    const data = await response.json();
    if (!response.ok) {
